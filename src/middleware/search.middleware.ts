...
export const getSearchSymbols = async (
  ...
  return fetch(
    `${process.env.REACT_APP_BASE_URL}${SEARCH_TERM_URL}${searchTerm}&apikey=${process.env.REACT_APP_STOCKS_API_KEY3}`,
...
export const getStockDetailsFromSymbol = async (
  ...
  return fetch(
    `${process.env.REACT_APP_BASE_URL}${GET_STOCK_DETAILS_URL}${symbol}&apikey=${process.env.REACT_APP_STOCKS_API_KEY3}`,
  ).then(parseFetchResponse);
};
